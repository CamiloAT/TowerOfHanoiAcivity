...
public class GamePanel extends JPanel{
	...
	private void initComponenets(ActionListener actionListener) {
		...
		for (int i = 0; i < size; i++) {
			...
			int a  = (temp.getWidth()/2);
			int b = (temp.getHeight()*i);
			percentageY.add(i, y+b);
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.drawLine(this.getPercentageValueWidth(1, 5), this.getPercentageValueHeight(1, 1), this.getPercentageValueWidth(1, 5), this.getPercentageValueHeight(1, 2));
		g2d.drawLine(this.getPercentageValueWidth(1, 2), this.getPercentageValueHeight(1, 1), this.getPercentageValueWidth(1, 2), this.getPercentageValueHeight(1, 2));
		g2d.drawLine(this.getPercentageValueWidth(4, 5), this.getPercentageValueHeight(1, 1), this.getPercentageValueWidth(4, 5), this.getPercentageValueHeight(1, 2));
	...
	public DiskPanel getOutDisk(int numberTowerToGet) {
		...
		diskUp=disk;
		disk.setLocation(this.getPositionToIncrease(numberTowerToGet), this.getPercentageValueHeight(40)-disk.getHeight());
		Vector<DiskPanel> stickToRemove = this.selectTower(numberTowerToGet);
		stickToRemove.remove(disk);
		counter++;
		return disk;
	}

	public void moveDisk(int numberTowerToMove, boolean flag) {
		DiskPanel disk = this.diskUp;
		Vector<DiskPanel> stickToMove = this.selectTower(numberTowerToMove);
		if (flag) {
			stickToMove.add(disk);
			if (numberTowerToMove==1) {
				disk.setLocation(this.getPercentageValueWidth(1, 5)-((disk.getWidth()/2)), this.percentageY.get(this.level-1-stackStick1.indexOf(disk)));
			}else if(numberTowerToMove==2) {
				disk.setLocation(this.getPercentageValueWidth(1, 2)-((disk.getWidth()/2)), this.percentageY.get(this.level-1-stackStick2.indexOf(disk)));
			}else if(numberTowerToMove==3) {
				disk.setLocation(this.getPercentageValueWidth(4, 5)-((disk.getWidth()/2)), this.percentageY.get(this.level-1-stackStick3.indexOf(disk)));
			}
			counter++;
		}
	}
